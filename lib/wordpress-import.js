...
// Parses a leading YYYY-MM-DD date from a post title (bloghog's convention: "2025-12-07 Some Hike").
// Returns the date string, or null if the title doesn't start with a valid calendar date.
function parseDateFromTitle(title) {
  ...
  // Date silently rolls invalid days (e.g. 2025-02-30) into the next month rather than erroring.
  if (date.getFullYear() !== Number(y) || date.getMonth() + 1 !== Number(mo) || date.getDate() !== Number(d)) return null;
  return `${y}-${mo}-${d}`;
}
...
module.exports = { wpApiUrlFor, fetchWpPost, parseContentBlocks, downloadImage, parseDateFromTitle };
